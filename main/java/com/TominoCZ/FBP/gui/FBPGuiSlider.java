...
public class FBPGuiSlider extends GuiButton {
	...
	public void drawButton(Minecraft mc, int mouseX, int mouseY) {
		...
		if (dragging) {
			...
			value = MathHelper.clamp_float(MathHelper.abs((float) (val / (width - 30))), 0, 1);
		}

		GL11.glBlendFunc(GL11.GL_SRC_ALPHA, GL11.GL_ONE_MINUS_SRC_ALPHA);

		this.drawTexturedModalRect((int) sliderPosX - 15, this.yPosition, 0, 100 + j * 20, 15, this.height);
		this.drawTexturedModalRect((int) sliderPosX, this.yPosition, 185, 100 + j * 20, 15, this.height);
	}
