...
public class FBPGuiFastAdd extends GuiScreen {
	...
	public FBPGuiFastAdd(FBPBlockPos selected) {
		...
		Block b = mc.theWorld.getBlockState(selected.getBlockPos()).getBlock();

		selectedBlock = b;

		ItemStack is = b.getPickBlock(mc.objectMouseOver, mc.theWorld, selected.getBlockPos(), mc.thePlayer);
	...
	public void updateScreen() {
		...
		if (selectedPos != null
			...
			Block b = mc.theWorld.getBlockState(mc.objectMouseOver.getBlockPos()).getBlock();
			...
			if (selected != null) {
				if (selected.enabled) {
					...
					mc.getSoundHandler().playSound(
							PositionedSoundRecord.create(new ResourceLocation("gui.button.press"), 1));
				}
			}
	...
	public void drawScreen(int mouseX, int mouseY, float partialTicks) {
		...
		this.itemRender.renderItemAndEffectIntoGUI(displayItemStack, 0,
				0);
		...
		String itemName = "";
		if (b != null) {
			itemName = Block.blockRegistry.getNameForObject(b).toString();
