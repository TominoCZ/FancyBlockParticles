...
public class FBPGuiMenuPage1 extends GuiScreen {
	...
	public void initGui() {
		...
		MinDurationMinus = new FBPGuiButton(2, x2, (int) MinDurationPlus.y, "-", false, false);

		MaxDurationPlus = new FBPGuiButton(3, x1, (int) MinDurationMinus.y + MinDurationMinus.height + 1, "+",
				false, false);
		MaxDurationMinus = new FBPGuiButton(4, x2, (int) MinDurationMinus.y + MinDurationMinus.height + 1, "-",
				false, false);

		InfiniteDuration = new FBPGuiButton(11, x1 + 25, (int) MinDurationPlus.y + 10,
				(FBP.infiniteDuration ? "\u00A7a" : "\u00A7c") + "\u221e", false, false);

		ScaleMultPlus = new FBPGuiButton(5, x1,
				(int) MaxDurationPlus.y + MaxDurationPlus.height + 6 + MaxDurationPlus.height / 2, "+", false,
				false);
		ScaleMultMinus = new FBPGuiButton(6, x2,
				(int) MaxDurationPlus.y + MaxDurationPlus.height + 6 + MaxDurationPlus.height / 2, "-", false,
				false);
		GravitiyForcePlus = new FBPGuiButton(7, x1,
				(int) ScaleMultMinus.y + ScaleMultPlus.height / 2 + 1 + ScaleMultMinus.height + 6, "+", false,
				false);
		GravitiyForceMinus = new FBPGuiButton(8, x2,
				(int) ScaleMultMinus.y + ScaleMultPlus.height / 2 + 1 + ScaleMultMinus.height + 6, "-", false,
				false);

		RotSpeedPlus = new FBPGuiButton(9, x1, (int) GravitiyForceMinus.y + GravitiyForceMinus.height + 1, "+",
				false, false);
		RotSpeedMinus = new FBPGuiButton(10, x2, (int) GravitiyForceMinus.y + GravitiyForceMinus.height + 1,
				"-", false, false);

		Defaults = new FBPGuiButton(0, this.width / 2 + 2, RotSpeedMinus.y + RotSpeedMinus.height + 24,
				"Defaults", false, false);
		Done = new FBPGuiButton(-1, x2, (int) Defaults.y, "Done", false, false);
		Reload = new FBPGuiButton(-2, x2, (int) Defaults.y + Defaults.height + 1, "Reload Config", false,
				false);
		...
		Reload.width = 96 * 2 + 8;
		Next = new FBPGuiButton(-3, RotSpeedMinus.x + RotSpeedMinus.width + 3 + 2 + 20,
				(int) RotSpeedMinus.y + 10, ">>", false, false);
	...
	public void drawScreen(int mouseX, int mouseY, float partialTicks) {
		FBPGuiHelper.background(MinDurationPlus.y - 6, Done.y - 4, width, height);

		drawInfo();

		drawMouseOverSelection(mouseX, mouseY, partialTicks);

		FBPGuiHelper.drawTitle(MinDurationPlus.y, width, height, fontRenderer);
	...
	private void drawMouseOverSelection(int mouseX, int mouseY, float partialTicks) {
		...
		if (((mouseX >= MinDurationMinus.x + MinDurationMinus.width) && (mouseX <= MinDurationPlus.x))
				&& (mouseY >= MinDurationMinus.y)
				&& (mouseY <= MaxDurationMinus.y + MaxDurationMinus.height - 2)) {
			handle.y = MinDurationPlus.y;
			size = new Vector2d(MinDurationPlus.x - (MinDurationMinus.x + MinDurationMinus.width), 39);
			selected = 1;
			mouseOver = true;
		} else if (((mouseX >= ScaleMultMinus.x + ScaleMultMinus.width) && (mouseX <= ScaleMultPlus.x))
				&& (mouseY >= (ScaleMultMinus.y + 1))
				&& (mouseY <= (ScaleMultPlus.y + ScaleMultPlus.height - 1) - 1)) {
			handle.y = ScaleMultPlus.y;
			size = new Vector2d(ScaleMultPlus.x - (ScaleMultMinus.x + ScaleMultMinus.width), 18);
			selected = 2;
		} else if (((mouseX >= GravitiyForceMinus.x + GravitiyForceMinus.width)
				&& (mouseX <= GravitiyForcePlus.x)) && (mouseY >= GravitiyForceMinus.y + 1)
				&& (mouseY <= GravitiyForcePlus.y + GravitiyForcePlus.height - 1)) {
			handle.y = GravitiyForceMinus.y;
			size = new Vector2d(GravitiyForcePlus.x - (GravitiyForceMinus.x + GravitiyForceMinus.width),
					18);
			selected = 3;
		} else if (((mouseX >= RotSpeedMinus.x + RotSpeedMinus.width) && (mouseX <= RotSpeedPlus.x))
				&& (mouseY >= RotSpeedMinus.y + 1)
				&& (mouseY <= RotSpeedPlus.y + RotSpeedMinus.height - 1)) {
			handle.y = RotSpeedMinus.y;
			size = new Vector2d(RotSpeedPlus.x - (RotSpeedMinus.x + RotSpeedMinus.width), 18);
			selected = 4;
		...
		if (lastHandle != new Vector2d(0, 0)) {
			...
			lastHandle.x = MinDurationMinus.x + MinDurationPlus.width;
		}
		...
		if (lastSize != new Vector2d(0, 0)) {
			...
			lastSize.x = GravitiyForcePlus.x - (GravitiyForceMinus.x + GravitiyForceMinus.width);
		}
		...
		if (((mouseX >= MinDurationMinus.x + MinDurationMinus.width && mouseX <= MinDurationPlus.x)
				&& (mouseY < RotSpeedMinus.y + RotSpeedMinus.height && mouseY >= MinDurationMinus.y)
				&& (lastSize.y <= 20 || (lastSize.y < 50 && lastSize.y > 20))
				&& lastHandle.y >= MinDurationPlus.y) || InfiniteDuration.isMouseOver()) {
			moveText(text);

			if (selected == 1)
				this.drawCenteredString(fontRenderer, !FBP.showInMillis ? "show in \u00A7ams" : "show in \u00A7aticks",
						this.width / 2, MinDurationPlus.y + MinDurationPlus.width - 5,
	...
	private void drawInfo() {
		...
		this.drawCenteredString(fontRenderer, s, this.width / 2, MinDurationPlus.y + 6,
