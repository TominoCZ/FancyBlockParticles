...
public class FBPGuiMenuPage1 extends GuiScreen {
	...
	public void initGui() {
		...
		Reload.width = 96 * 2 + 8;
		Next = new FBPGuiButton(-3, RotSpeedMinus.xPosition + RotSpeedMinus.width + 3 + 2 + 20,
				(int) RotSpeedMinus.yPosition + 10, ">>", false, false);
	...
	private void drawMouseOverSelection(int mouseX, int mouseY, float partialTicks) {
		...
		if (((mouseX >= MinDurationMinus.xPosition + MinDurationMinus.width && mouseX <= MinDurationPlus.xPosition)
			...
			if (selected == 1)
				this.drawCenteredString(fontRendererObj, !FBP.showInMillis ? "show in \u00A7ams" : "show in \u00A7aticks",
