...
public class FBPGuiHelper extends GuiScreen {
	...
	protected static void overlayBackground(int startY, int endY, int startAlpha, int endAlpha, int top, int bottom,
		...
		BufferBuilder.begin(7, DefaultVertexFormats.POSITION_TEX_COLOR);
		BufferBuilder.pos(left, endY, 0.0D).tex(0.0D, endY / 32.0F)
				.color(64, 64, 64, endAlpha).endVertex();
		BufferBuilder.pos(left + right, endY, 0.0D)
				.tex(right / 32.0F, endY / 32.0F).color(64, 64, 64, endAlpha)
				.endVertex();
		BufferBuilder.pos(left + right, startY, 0.0D)
				.tex(right / 32.0F, startY / 32.0F).color(64, 64, 64, startAlpha)
				.endVertex();
		BufferBuilder.pos(left, startY, 0.0D).tex(0.0D, startY / 32.0F)
	...
	protected static void drawContainerBackground(Tessellator tessellator, int top, int bottom, int left, int right) {
		...
		buffer.pos(left, bottom, 0.0D).tex(left / f, bottom / f)
				.color(32, 32, 32, 255).endVertex();
		buffer.pos(right, bottom, 0.0D).tex(right / f, bottom / f)
				.color(32, 32, 32, 255).endVertex();
		buffer.pos(right, top, 0.0D).tex(right / f, top / f)
				.color(32, 32, 32, 255).endVertex();
		buffer.pos(left, top, 0.0D).tex(left / f, top / f)
				.color(32, 32, 32, 255).endVertex();
