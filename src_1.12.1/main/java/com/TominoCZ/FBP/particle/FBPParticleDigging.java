...
public class FBPParticleDigging extends ParticleDigging {
		...
		if (scale < -1) {
			if (facing != null) {
				...
				if (facing == EnumFacing.UP && FBP.smartBreaking) {
					...
					double x = FBPMathUtil.add(cameraViewDir.x, 0.001D);
					double z = FBPMathUtil.add(cameraViewDir.z, 0.001D);
	...
	public void moveEntity(double x, double y, double z, boolean YOnly) {
		...
		if (this.canCollide) {
			...
			this.setBoundingBox(this.getBoundingBox().offset(YOnly ? 0.0D : x, y, YOnly ? 0.0D : z));
		} else
	...
	public void putCube(BufferBuilder worldRendererIn, double scale, FBPVector3d rotVec, int j, int k, float r, float g,
