...
public class FBPEntityRenderer extends EntityRenderer {
	...
	public void updateRenderer() {
		...
		if (FBP.fancyWeather && ++tick >= 15) {
			BlockPos.MutableBlockPos blockpos$mutableblockpos = new BlockPos.MutableBlockPos();
			int r = mc.gameSettings.renderDistanceChunks * 9;
			...
			double offsetY = mc.thePlayer.motionY * 6;
			...
			for (int i = 0; i < 45; i++) {
				...
				if (biome.getEnableSnow()) {
					mc.effectRenderer.addEffect(new FBPParticleSnow(mc.theWorld, X, mc.thePlayer.posY + 15 + offsetY, Z,
							FBP.random.nextDouble(-0.5, 0.5), FBP.random.nextDouble(0.65, 0.85) + mT,
							FBP.random.nextDouble(-0.5, 0.5), Blocks.SNOW.getDefaultState()));
				}
			}

			tick = (int) (mT * 2 + offsetY / 2);
		}
	}
						...
						if (k2 != l2) {
							...
							if (world.getBiomeProvider().getTemperatureAtHeight(f2, j2) >= 0.15F) {
								...
								vertexbuffer.pos((double) l1 - d3 + 0.5D, (double) k2, (double) k1 - d4 + 0.5D)
										...
										.lightmap(k3, l3).endVertex();
							} else if (!FBP.enabled || !FBP.fancyWeather) {
								if (j1 != 1) {
									if (j1 >= 0) {
										tessellator.draw();
									}
