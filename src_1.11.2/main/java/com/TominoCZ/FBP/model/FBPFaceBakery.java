...
import com.TominoCZ.FBP.util.FBPRenderUtils;
...
public class FBPFaceBakery extends FaceBakery {
    public BakedQuad makeBakedQuad(Vector3f min, Vector3f max, int tintIndex,
        ...
        return makeBakedQuad(min, max, tintIndex, FBPRenderUtils.calculateUV(min, max, facing), icon, facing, rot, uvLocked);
    }
    ...
    public BakedQuad makeBakedQuad(Vector3f min, Vector3f max, int tintIndex, float[] uv,
        ...
        if (hasColor) {
            FBPRenderUtils.recolorQuad(quad, tintIndex);
        }
